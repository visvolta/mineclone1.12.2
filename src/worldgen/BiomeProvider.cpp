...
constexpr BiomeDefinition biome(int id, float base, float variation, float temperature = 0.5F,
    ...
    return {id, base, variation, temperature, rainfall, snowy};
}
...
const std::array<BiomeDefinition, 256>& biomeTable() {
    static const std::array<BiomeDefinition, 256> table = [] {
        ...
        result[6] = biome(6, -0.2F, 0.1F, 0.8F, 0.9F); result[7] = biome(7, -0.5F, 0.0F);
        ...
        result[133] = biome(133, 0.3F, 0.4F, 0.25F, 0.8F); result[134] = biome(134, -0.1F, 0.3F, 0.8F, 0.9F);
