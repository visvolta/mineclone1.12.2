...
GLuint compileShader(GLenum type,const char* source){
    ...
    if(!ok){ glDeleteShader(shader); throw std::runtime_error("Entity shader compile failed"); }
    return shader;
}
...
EntityRenderer::EntityRenderer(const std::filesystem::path& root,const ItemRegistry& items,
    ...
    const auto xpPath=assetRoot_/"assets/minecraft/textures/entity/experience_orb.png";
    int w=0,h=0,c=0; unsigned char* pixels=stbi_load(xpPath.string().c_str(),&w,&h,&c,STBI_rgb_alpha);
    if(!pixels||w!=64||h!=64){if(pixels)stbi_image_free(pixels);throw std::runtime_error("Missing/invalid Minecraft 1.12.2 XP orb texture: "+xpPath.string());}
    glGenTextures(1,&xpTexture_); glBindTexture(GL_TEXTURE_2D,xpTexture_);
    glTexImage2D(GL_TEXTURE_2D,0,GL_RGBA8,w,h,0,GL_RGBA,GL_UNSIGNED_BYTE,pixels);
    glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MIN_FILTER,GL_NEAREST);glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_MAG_FILTER,GL_NEAREST);
    glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_WRAP_S,GL_CLAMP_TO_EDGE);glTexParameteri(GL_TEXTURE_2D,GL_TEXTURE_WRAP_T,GL_CLAMP_TO_EDGE);
    stbi_image_free(pixels);
    const std::array<Vertex,6> xp={{{-0.25F,-0.25F,0,0,0.25F,1},{0.25F,-0.25F,0,0.25F,0.25F,1},{0.25F,0.25F,0,0.25F,0,1},{-0.25F,-0.25F,0,0,0.25F,1},{0.25F,0.25F,0,0.25F,0,1},{-0.25F,0.25F,0,0,0,1}}};
    glGenVertexArrays(1,&xpVao_);glGenBuffers(1,&xpVbo_);glBindVertexArray(xpVao_);glBindBuffer(GL_ARRAY_BUFFER,xpVbo_);
    glBufferData(GL_ARRAY_BUFFER,sizeof(xp),xp.data(),GL_STATIC_DRAW);
...
EntityRenderer::~EntityRenderer(){
    ...
    if(xpVbo_)glDeleteBuffers(1,&xpVbo_); if(xpVao_)glDeleteVertexArrays(1,&xpVao_); if(xpTexture_)glDeleteTextures(1,&xpTexture_);
    if(program_)glDeleteProgram(program_);
}

std::uint32_t EntityRenderer::geometryKey(const ItemStack& stack) const {
    return (static_cast<std::uint32_t>(stack.itemId)<<16U)|static_cast<std::uint32_t>(stack.damage);
}

const EntityRenderer::Geometry& EntityRenderer::geometryFor(const ItemStack& stack){
...
void EntityRenderer::render(const EntityManager& system,const glm::mat4& view,const glm::mat4& projection,float partial){
    ...
    for(const auto& owned:system.entities()){
        if(!owned || owned->removed) continue;
        const Entity& base=*owned;
        ItemStack renderStack{};
        float hover=0.0F;
        float yaw=base.previousYaw+(base.yaw-base.previousYaw)*std::clamp(partial,0.0F,1.0F);
        float bob=0.0F;
        int copies=1;
        bool spin=false;
        if(const auto* item=dynamic_cast<const EntityItem*>(&base)){
            renderStack=item->stack; hover=item->hoverStart;
            bob=std::sin((static_cast<float>(item->age)+partial)/10.0F+hover)*0.1F+0.1F;
            yaw=((static_cast<float>(item->age)+partial)/20.0F+hover)*57.2957795F;
            copies=modelCount(renderStack.count); spin=true;
        }else if(const auto* falling=dynamic_cast<const EntityFallingBlock*>(&base)){
            renderStack=items_.stackForBlock(falling->blockState,1);
        }else if(dynamic_cast<const EntityTNTPrimed*>(&base)){
            renderStack=items_.stackForBlock(makeBlockState(static_cast<std::uint16_t>(BlockId::TNT)),1);
        }else if(dynamic_cast<const EntityArrow*>(&base)){
            renderStack=ItemStack{262,1,0,{}};
        }else if(const auto* boat=dynamic_cast<const EntityBoat*>(&base)){
            renderStack=ItemStack{333,1,static_cast<std::uint16_t>(std::clamp(boat->boatType,0,5)),{}};
        }else if(dynamic_cast<const EntityMinecart*>(&base)){
            renderStack=ItemStack{328,1,0,{}};
        }else if(dynamic_cast<const EntityXPOrb*>(&base)){
            const glm::dvec3 ip=base.interpolatedPosition(partial);
            glm::mat4 billboard(1.0F);
            for(int i=0;i<3;++i)for(int j=0;j<3;++j)billboard[i][j]=view[j][i];
            glm::mat4 model=glm::translate(glm::mat4(1.0F),glm::vec3(ip)+glm::vec3(0,0.25F,0))*billboard;
            const glm::mat4 mvp=projection*view*model;
            glUniformMatrix4fv(glGetUniformLocation(program_,"mvp"),1,GL_FALSE,glm::value_ptr(mvp));
            glActiveTexture(GL_TEXTURE0);glBindTexture(GL_TEXTURE_2D,xpTexture_);glBindVertexArray(xpVao_);glDrawArrays(GL_TRIANGLES,0,6);
            continue;
        }else continue;
        if(renderStack.empty()) continue;
        const Geometry& geometry=geometryFor(renderStack);
        if(geometry.vertexCount<=0) continue;
        const glm::dvec3 interpolated=base.interpolatedPosition(partial);
        std::mt19937 random(static_cast<unsigned>(renderStack.itemId+renderStack.damage));
        std::uniform_real_distribution<float> offset(-0.15F,0.15F);
        glActiveTexture(GL_TEXTURE0); atlas_.bind(0);
        for(int k=0;k<copies;++k){
            glm::mat4 model(1.0F);
            model=glm::translate(model,glm::vec3(interpolated)+glm::vec3(0,bob+0.25F,0));
            if(spin||std::abs(yaw)>0.001F) model=glm::rotate(model,glm::radians(yaw),glm::vec3(0,1,0));
            if(k>0){
            ...
            const float scale = dynamic_cast<const EntityArrow*>(&base) ? 0.55F : (dynamic_cast<const EntityBoat*>(&base)||dynamic_cast<const EntityMinecart*>(&base) ? 1.25F : 0.8F);
